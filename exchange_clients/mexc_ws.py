    ...
    try:
        async for message in websocket:
            try:
                ...
                logger.debug(f"[{MEXC_EXCHANGE_NAME}] Получено raw сообщение: {data}")

                if not isinstance(data, dict):
                    logger.warning(f"[{MEXC_EXCHANGE_NAME}] Сообщение не словарь: {data}")
                    continue
                ...
                if "result" in data:
                    if data["result"] == "success":
                        logger.info(f"[{MEXC_EXCHANGE_NAME}] Успешный ответ (result): {data}")
                    else:
                        logger.warning(f"[{MEXC_EXCHANGE_NAME}] Ответ с ошибкой (result): {data}")
                    is_response = True
                elif "code" in data and data["code"] != 0:
                    logger.warning(f"[{MEXC_EXCHANGE_NAME}] Ответ с кодом ошибки (code): {data}")
                    is_response = True
                elif "msg" in data:
                    ...
                    if "success" in msg_lower or "subscribe" in msg_lower or "spot@public" in msg_lower:
                        ...
                        logger.warning(f"[{MEXC_EXCHANGE_NAME}] Ответ с ошибкой (msg): {data}")
                        is_response = True
                if is_response:
                    continue
                ...
                if channel.startswith("spot@public.bookTicker.v3.api@") and "d" in data:
                    ...
                    if not symbol or symbol not in SYMBOLS_TO_TRACK:
                        if logger.isEnabledFor(logging.DEBUG):
                             logger.debug(f"[{MEXC_EXCHANGE_NAME}] Пропуск тикера: {symbol}")
                        continue

                    try:
                        bid_price = Decimal(ticker_data.get("b")) if ticker_data.get("b") else None
                        ask_price = Decimal(ticker_data.get("a")) if ticker_data.get("a") else None
                        timestamp_ms = int(data.get("t", 0))

                        if bid_price is None or ask_price is None:
                            logger.warning(f"[{MEXC_EXCHANGE_NAME}][{symbol}] Отсутствуют bid/ask: {ticker_data}")
                            continue
                        ...
                        continue
                         ...
                         logger.warning(f"[{MEXC_EXCHANGE_NAME}][{symbol}] Ошибка обработки BBO: {e} - Tick: {ticker_data}")
                    except Exception as e_inner:
                        logger.error(f"[{MEXC_EXCHANGE_NAME}][{symbol}] Ошибка при обработке BBO: {e_inner}", exc_info=True)
        ...
        logger.warning(f"[{MEXC_EXCHANGE_NAME}] Соединение закрыто: Код={e.code}, Причина='{e.reason}'")
    except Exception as e:
        logger.error(f"[{MEXC_EXCHANGE_NAME}] Критическая ошибка в цикле handle_mexc_messages: {e}", exc_info=True)
    finally:
    ...
    while True:
        try:
            ...
            async with websockets.connect(
                uri,
                ping_interval=None, # Отвечаем вручную
                ping_timeout=40
                ) as websocket:
                logger.info(f"[{MEXC_EXCHANGE_NAME}] WebSocket соединение установлено: {websocket.id}")
            ...
            logger.error(f"[{MEXC_EXCHANGE_NAME}] Ошибка/таймаут соединения WebSocket: {e}")
        except Exception as e:
            logger.error(f"[{MEXC_EXCHANGE_NAME}] Непредвиденная ошибка в mexc_client: {e}", exc_info=True)
